  ...
  if (methodsUsed > 1) {
    return done(error('invalid_request',
      'Only one method may be used to authenticate at a time (Auth header,  ' +
        'GET or POST).'));
  } else if (methodsUsed === 0 && this.config.strict) {
    // Access token is always required in strict mode
...
function checkToken (done) {
  ...
  // Pass through when no access token provided in non-strict mode
  if (!this.config.strict && !this.bearerToken) {
    return done();
  }
  ...
  this.model.getAccessToken(this.bearerToken, function (err, token) {
    ...
    self.req.user = token.user ? token.user : { id: token.userId, roles: token.roles, scope: token.scope };
