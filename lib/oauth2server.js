...
function OAuth2Server (config) {
  ...
  // Strict mode. Default to true to always requires access token.
  this.strict = typeof config.strict === 'undefined' ? true : config.strict;
}
 ...
 */
OAuth2Server.prototype.authorise = function (config) {
  var self = this;

  return function (req, res, next) {
    return new Authorise(extend(self, config || {}), req, next);
  };
};
